...
class SoftmaxCrossEntropyWithLogits():
    ...
    def forward(self, x, y):
        ...
        return -np.mean(np.log(probabilities[range(len(probabilities)), np.argmax(y, axis=1)]))
    ...
    def backward_inputs(self, x, y):
        ...
        """
        return np.divide(self._softmax(x) - y, x.shape[0])
...
class L2Regularizer():
    ...
    def forward(self):
        ...
        """
        return np.divide(self.weight_decay * np.square(np.linalg.norm(self.weights)), 2)
